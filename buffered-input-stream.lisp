...
(defmethod initialize-instance :after ((buffered-stream buffered-character-input-stream)
                                       &key
                                         (stream nil)
                                         (size 4096)
  ...
  (setf (slot-value buffered-stream 'buffer) (make-stream-ring-buffer :stream (make-instance 'character-indexed-input-stream :stream stream) :buffer buffer)))
...
(defmethod initialize-instance :after ((buffered-stream buffered-binary-input-stream)
                                       &key
                                         (stream nil)
                                         (size 4096)
                                       ...
                                       &allow-other-keys)
  (assert (input-stream-p stream))
  (setf (slot-value buffered-stream 'buffer) (make-stream-ring-buffer :stream stream :buffer buffer)))
